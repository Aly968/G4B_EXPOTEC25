...
public class Unirse_a_empresa extends JFrame implements ActionListener {
    ...
    public void posicionar() {
        ...
        btnunirse.setBounds(160, 220, 200, 35);
    ...
    private void finalizarRegistro() {
        //Obtiene el id ingresado por el usuario sin espacios
        String idEmpresa = cod.getText().trim();
        
        //Valida que el texto no este en blanco
        if (idEmpresa.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Ingrese el ID de empresa.", "Advertencia", JOptionPane.WARNING_MESSAGE);
            return;
        }

        //Valida que el texto ingresado sean solo entero
        int idEm;
        try {
            idEm = Integer.parseInt(idEmpresa);
        } catch (NumberFormatException e) {
            JOptionPane.showMessageDialog(this, " Ingrese código válido.", "Error", JOptionPane.ERROR_MESSAGE);
            cod.setText("");
            return;
        }
        ...
        //Maneja instrucción que verifica la existencia de empresa
        PreparedStatement stmt = null;
        ResultSet rs = null;
        PreparedStatement stmtU = null;

        //Manejo de consulta que consulta base de datos y obtiene la cuenta de el ID ingresado, verifica si existe la emepresa
        try {
            ...
            if (dato == 0) {
                JOptionPane.showMessageDialog(this, "ID no válido, ingrese empresa registrada.", "Error", JOptionPane.ERROR_MESSAGE);
                cod.setText("");
                return;
            }
            
            //Si se completa lo anterior procede con la actualización de la tabla usuarios
            ...
            //Obtiene el numero de filas modificadas, en caso exitoso es 1
            int rowsUp = stmtU.executeUpdate();
            //Si es exitoso cierra el proceso con el commit guardando los registros definiticamente
            if (rowsUp > 0) {
                conn.commit();
            ...
            JOptionPane.showMessageDialog(this, "Error al finalizar registro.");
            e.printStackTrace();
