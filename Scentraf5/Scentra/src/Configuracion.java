...
//metodo,copia y pega
   ...
   private void actualizarLogoDisplay(String logoFileName) {
    ...
    if (logoFileName != null && !logoFileName.isEmpty()) {
        imagePath = System.getProperty("user.dir") + File.separator + logoFileName;

        File logoFile = new File(imagePath);
        if (!logoFile.exists()) {
            imagePath = null; // Si el archivo no existe, no intentes cargarlo
        }
    }
    ...
    if (imagePath != null) {
        try {
            newLogoIcon = new ImageIcon(imagePath);
            // Verifica si la imagen se cargó completamente
            if (newLogoIcon.getImageLoadStatus() != MediaTracker.COMPLETE) {
                newLogoIcon = null; // No se cargó bien, forzar a usar default
            }
        } catch (Exception e) {
            newLogoIcon = null; // En caso de cualquier otro error al cargar
        }
    }

    // Si newLogoIcon sigue siendo nulo (no se pudo cargar el logo específico),
    // intenta cargar el logo por defecto.
    if (newLogoIcon == null || newLogoIcon.getImage() == null) {
        try {
            ImageIcon defaultIcon = new ImageIcon("default_logo.png");
            if (defaultIcon.getImageLoadStatus() == MediaTracker.COMPLETE) {
                newLogoIcon = defaultIcon;
            } else {
                newLogoIcon = new ImageIcon(); // Icono vacío si ni el default carga
            }
        } catch (Exception e) {
            newLogoIcon = new ImageIcon(); // En caso de error al cargar el default
        }
    }

    if (pR != null) {
        if (newLogoIcon != null && newLogoIcon.getImage() != null) {
            // Asegúrate de que el tamaño de pR esté definido o usa un tamaño por defecto
            int targetWidth = pR.getWidth() > 0 ? pR.getWidth() : 280;
            int targetHeight = pR.getHeight() > 0 ? pR.getHeight() : 360;
            Image scaledImagePR = newLogoIcon.getImage().getScaledInstance(
                    targetWidth, targetHeight, Image.SCALE_SMOOTH);
            pR.setIcon(new ImageIcon(scaledImagePR));
        } else {
            pR.setIcon(null); // Si no hay icono para mostrar, establece a null
        }
        pR.revalidate();
        pR.repaint();
    }
        ...
        if (e.getSource() == btnDashboard) {
            ...
            if (result == JFileChooser.APPROVE_OPTION) {
                ...
                if (newLogoFileName != null) {
                    // Solo se actualiza pR aquí, sin afectar pR2
