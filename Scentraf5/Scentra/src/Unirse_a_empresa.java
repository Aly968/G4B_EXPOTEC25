...
public class Unirse_a_empresa extends JFrame implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
     ...
     private void finalizarRegistro() {
        // Obtiene el código de empresa ingresado por el usuario sin espacios
        String codigoEmpresa = cod.getText().trim();

        // Valida que el texto no esté en blanco
        ...
        int idEmpresaEncontrado = -1; // Variable para almacenar el idEmpresa encontrado

        try {
            String sql = "SELECT idEmpresa FROM Empresa WHERE Codigo_empresa = ?";
            stmt = conn.prepareStatement(sql);
            stmt.setString(1, codigoEmpresa); // Usar setString para Codigo_empresa
            rs = stmt.executeQuery();

            if (rs.next()) {
                idEmpresaEncontrado = rs.getInt("idEmpresa"); // Obtiene el idEmpresa
            } else {
                // Si no se encuentra ninguna empresa con ese código
            ...
            // Si se encuentra el idEmpresa, procede con la actualización de la tabla Usuarios
            String sqlUp = "UPDATE Usuarios SET idEmpresa = ?, Estado_Union_Empresa = 'Activo' WHERE idUsuarios = ?";
            stmtU = conn.prepareStatement(sqlUp);
            stmtU.setInt(1, idEmpresaEncontrado); // Usa el idEmpresa encontrado
            stmtU.setInt(2, idUsuario);
            
            // Obtiene el número de filas modificadas, en caso exitoso es 1
            ...
            if (rowsUp > 0) {
                ...
                new Inicio_de_sesion(); // Asumiendo que esta clase existe y maneja el inicio de sesión
            ...
            try {
                ...
                ex.printStackTrace(); // Imprime la pila de errores del rollback
            }
            JOptionPane.showMessageDialog(this, "Error al finalizar registro: " + e.getMessage(), "Error", JOptionPane.ERROR_MESSAGE);
            e.printStackTrace(); // Imprime la pila de errores original
        } finally {
            // Cierra los recursos de la base de datos en el bloque finally para asegurar que se cierren
            try {
                if (rs != null) {
                    rs.close();
                }
