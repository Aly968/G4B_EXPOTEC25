...
//Gabriel Ricardo Rodriguez de León
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (e.getSource() == iniciar) {
            ...
            String nombreUsuario = Usuario.getText().trim(); // .trim elimina espacios en blanco
            String contrasena = new String(Contrasena.getPassword()); // Convierte a string el valor ingresado
            String rol = (String) Rol.getSelectedItem(); // Convierte a string
            ...
            if (nombreUsuario.isEmpty() || contrasena.isEmpty() || rol == null || rol.isEmpty()) {
                ...
                return; // Vuelve a ejecutar el proceso
            }
            ...
            try {
                // Crea un objeto de clase conexion y abre esa misma
                Conexion con = new Conexion(); // Asegúrate de que tu clase Conexion maneje correctamente la conexión
                conn = con.getConnection();
                // Permite el guardar temporalmente los datos hasta confirmar definitivamente la inserción al unirse a una empresa
                conn.setAutoCommit(false);

                // Genera un nuevo Id: obtiene el último ID y le suma 1
                int crearId = 1; // Valor inicial si no hay usuarios registrados
                Statement st = null;
                try {
                    ...
                    if (rs.next()) {
                        ...
                        if (ultimoId > 0) { // Si ya existen IDs, el nuevo será el último + 1
                            crearId = ultimoId + 1;
                        }
                    }
                } finally {
                    // Cerrar el ResultSet y Statement para la consulta del MAX(idUsuarios)
                    if (rs != null) {
                ...
                // Insertar el nuevo usuario con ID generado
                ...
                // Si el número de filas es mayor a 0 (debería ser 1 si se insertó el registro)
                if (rowsAffected > 0) {
                    JOptionPane.showMessageDialog(this, "Paso 1 Completado");

                    // Abre ventana de registro a empresa y pasa la conexión
                    ...
                    // Si no se insertaron filas, algo salió mal aunque no hubo una excepción SQL directa
